...
class MediaCacheManager(context: Context) {
    ...
    fun buildDataSourceFactory(): DataSource.Factory {
        Log.d(TAG, "Building cached data source factory with userAgent=$USER_AGENT")
    ...
    fun buildLiveDataSourceFactory(): DataSource.Factory {
        Log.d(TAG, "Building uncached live data source factory with userAgent=$USER_AGENT")
    ...
    fun release() {
        cache?.let {
            ...
            it.release()
        } ?: Log.d(TAG, "release() called with no active media cache")
        cache = null
    }
}
